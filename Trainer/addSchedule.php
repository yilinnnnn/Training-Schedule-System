...
$db=mysqli_connect('localhost','root',null,'training_system');
 if(isset($_POST['submit'])) { 
    ...
    //book quantity automatically decrease by 1 if a course is selected
    $course=$_POST['course'];
    $query="SELECT * FROM inventory WHERE course_title='$course'";
    $result3=mysqli_query($db,$query);
    $row=mysqli_fetch_array($result3);
    $quantity=$row['quantity'];
    $quantity-=1;
    $query="UPDATE inventory SET quantity='$quantity' WHERE course_title='$course'";
...
<div class="main">
    <form method="post" action=""> 
        <table width:80%>
            ...
            <tr><th>Training End Date:<th><input type="date" name="end_date" min=<?php echo date('Y-m-d'); ?>> </th></tr>
            ...
            <tr><th>Course:<th><select id="course" onclick="myFunction()" name="course" required>
